...
def binary_false_pos(s, g):
    ...
    prod = np.multiply(s, g)
    # Invert ground truth
    inverted_g = np.invert(g)
    false_pos = np.multiply(s, inverted_g)
    num_false_pos = false_pos.sum()  # number of false positive
    s0 = prod.sum()
    s1 = s.sum()
    s2 = g.sum()
    # dice = (2.0*s0 + 1e-10)/(s1 + s2 + 1e-10)
    pos_div_all = num_false_pos / s2
    return pos_div_all


def binary_false_neg(s, g):
    """
    dice score of 3d binary volumes
    inputs:
        s: segmentation volume
    ...
    prod = np.multiply(s, g)
    # Invert ground truth
    inverted_g = np.invert(s)
    false_pos = np.multiply(g, inverted_g)
    num_false_pos = false_pos.sum()  # number of false negative
    s0 = prod.sum()
    s1 = s.sum()
    s2 = g.sum()
    # dice = (2.0*s0 + 1e-10)/(s1 + s2 + 1e-10)
...
def ensemble_two_nets(s, g, gt):
    assert (len(s.shape) == 3)
    [Ds, Hs, Ws] = s.shape
    [Dg, Hg, Wg] = g.shape
    assert (Ds == Dg and Hs == Hg and Ws == Wg)
    prod = np.multiply(s, g)
    return binary_dice3d(prod, gt)


def ensemble_three_nets(f, s, t, gt):
    prod1 = np.multiply(f, s)
    prod2 = np.multiply(f, t)
    prod3 = np.multiply(s, t)
    prod = np.bitwise_or(np.bitwise_or(prod1, prod2), prod3)
    return binary_dice3d(prod, gt)
