...
def test(config_file, data_root):
    ...
    if (config_test.get('whole_tumor_only', False) is False):
        ...
        if (config_net3):
            ...
            # 4, load test images
    dataloader = DataLoader(config_data, data_root)
    ...
    for i in range(image_num):
        ...
        if (config_test.get('whole_tumor_only', False) is True):
            ...
            out_label = label1 * 2
            core_label = np.zeros(out_label.shape)
            enchanced_label = np.zeros(out_label.shape)
            if ('nii' in config_data['file_postfix']):
                core_label[label2 > 0] = 1
                enchanced_label[label3 > 0] = 4
            out_label = np.asarray(out_label, np.int16)
            core_label = np.asarray(core_label, np.int16)
            enchanced_label = np.asarray(enchanced_label, np.int16)
        ...
        save_array_as_nifty_volume(final_label, save_folder + "/{0:}_wh.nii.gz".format(temp_name), img_names[0])

        final_label = np.zeros(temp_size, np.int16)
        final_label = set_ND_volume_roi_with_bounding_box_range(final_label, temp_bbox[0], temp_bbox[1], core_label)
        save_array_as_nifty_volume(final_label, save_folder + "/{0:}_core.nii.gz".format(temp_name), img_names[0])

        final_label = np.zeros(temp_size, np.int16)
        final_label = set_ND_volume_roi_with_bounding_box_range(final_label, temp_bbox[0], temp_bbox[1],
                                                                enchanced_label)
        save_array_as_nifty_volume(final_label, save_folder + "/{0:}_ench.nii.gz".format(temp_name), img_names[0])
...
if __name__ == '__main__':
    ...
    data_root = str(sys.argv[2])
    assert (os.path.isfile(config_file))
    test(config_file, data_root)
